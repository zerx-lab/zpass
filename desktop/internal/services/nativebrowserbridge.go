...
func (s *BrowserBridgeServer) Start() error {
	...
	cfg := nativebridge.Config{
		Port:  strconv.Itoa(listener.Addr().(*net.TCPAddr).Port),
		Token: token,
	}
	configPath := filepath.Join(dir, nativebridge.ConfigFile)
	if err := nativebridge.WriteConfig(configPath, cfg); err != nil {
		_ = listener.Close()
		return err
	}
...
func (s *BrowserBridgeServer) handleNative(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, nativebridge.MaxMessageBytes)).Decode(&msg); err != nil {
