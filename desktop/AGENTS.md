...
- 打包主用 Electron Forge;Windows NSIS 向导安装包与自动更新元数据(latest.yml/blockmap)由 electron-builder `--prepackaged` 二次封装 Forge 产物(见「Windows 安装包」「自动更新」两节)。除此之外不要再引入第二套打包器。
...
`main.ts` 在 `app.setName` 之后、`app.whenReady()` 之前批量 `app.commandLine.appendSwitch` 设置 Chromium 开关：`disable-features=CalculateNativeWinOcclusion,Vulkan`（前者 Win-only 省 200–300ms，后者修 Wayland GPU process fatal crash 与 ~500ms swiftshader fallback 重试）、`disable-renderer-backgrounding`、`no-default-browser-check`，Linux 上额外 `ozone-platform-hint=auto` + `class=zpass`。**不要**把 Linux 上的 `ozone-platform` 硬钉成 `x11`：在 NVIDIA + Wayland 上 XWayland 的 GPU 进程会立刻段错误（`exit_code=139`，`XGetWindowAttributes failed`）导致窗口起不来。两条 Wayland 噪声 ERROR（`wayland_surface_factory.cc … not compatible with Vulkan` 与 `viz/.../display.cc: Frame latency is negative`）是已知无害的，接受它们换可启动性。新增开关一律放这一段，**不要**写进 `app.whenReady()` 回调——GPU/utility 子进程在那之前就已经 fork。
...
## Windows 安装包 / Node ≥26 兼容（打包链 load-bearing 修复）

下面三处是 `task make:win:nsis` 在 Node ≥26 上能产出安装包的前提；删任何一处打包都会失败，且报错位置离根因很远（曾耗时定位）。根因都是 Node v26 太新打破了古老的传递依赖，机器无 nvm/fnm/volta 时只能在依赖层修。

- **`package.json` 的 `pnpm.overrides` `"hosted-git-info@4>lru-cache": "^7.18.3"`** —— `node-linker=hoisted` 把 `yallist@5` 错放到 `app-builder-lib` 内嵌的 `lru-cache@6` 旁（后者要 `yallist@4` 的 `new Yallist()`），electron-builder 一启动就 `TypeError: Yallist is not a constructor`。把这条边的 lru-cache 提到无 yallist 依赖的 7.x 即从根上消除冲突（`tar@7` 仍合法用 `yallist@5`）。不能全局锁 yallist。
- **`patches/extract-zip@2.0.1.patch`** —— `@electron/packager` 用的 extract-zip 经 yauzl→fd-slicer 的 `fs.read` 分片流在 Node ≥26 下解压 Electron 模板时中途停摆（deflate entry 喂不到 EOF），Forge 静默 `exit 0`、不产出 `out/zpass-win32-x64`，随后 NSIS 步骤报 `ENOENT elevate.exe`（假象）。patch 改用 `yauzl.fromBuffer`（整包读入内存、不走 fs.read）并额外在 `'end'` 事件 resolve（buffer-backed zipfile 无 fd、不发 `'close'`，原代码只等 `'close'`）。改 extract-zip 版本需同步重做 patch。
- **`electron-builder.win.yml` 的 `toolsets.nsis: "1.2.1"`** —— electron-builder 26.15.2/26.15.3 的 legacy NSIS toolset（默认 `0.0.0`）去 `nsis-3.0.4.1` 根目录找 `elevate.exe`，但该归档把它放在 `Contrib/` 子目录 → `ENOENT elevate.exe`。统一 bundle（`nsis-bundle-3.12.tar.gz`）把 makensis/plugins/elevate 放在同一正确布局里。

## 自动更新（electron-updater + GitHub Releases，load-bearing 不变量）

Windows NSIS 走 electron-updater 全自动更新（后台下载 + 退出时安装 + 应用内「重启安装」提示，启动延迟自检 + 设置页手动按钮）；macOS（仅 ad-hoc 签名）与 Linux（AppImage 由 Forge 而非 electron-builder 构建、deb/rpm 归包管理器）**不做应用内安装**，仅 GitHub API 版本检查 + 打开下载页。代码在 `electron/src/main/updater.ts`（主进程）、`preload.ts` + `compat/window-globals.d.ts` 的 `update` 桥、`stores/update.ts` + `app/UpdateEventSync.tsx` + `AboutSection.tsx`（渲染层）。

- **`electron-updater` 必须被 Vite bundle 进主进程包**：Forge Vite plugin 不发布 `node_modules`（`out/.../resources/` 只有 `app.asar`+`bin`+`logo`）。**禁止**把它写进任何 `rollupOptions.external`，否则运行时 `Cannot find module`。
- **`resources/app-update.yml` 由 `electron/app-update.yml` 经 `forge.config.ts` 的 `extraResource` 自带**：electron-builder `--prepackaged` 模式跳过 app pack 阶段、**不会注入** app-update.yml，故必须自带（让 `setFeedURL` 之前能解析出一个合法 provider）。
- **`electron-builder.win.yml` 的 `artifactName` 固定为 `ZPass-windows-x64-Setup.exe`**：网站按固定资产名引用，且让生成的 `latest.yml` 的 `path` 与上传资产名一致（electron-updater 按 `latest.yml.path` 拉取，不一致即 404）。改名会同时断掉网站下载与自动更新。
- **`latest.yml` 必须随 Release 上传且不可改名**：由 `make:win:nsis`（`--publish never`，本地生成、CI 上传）产出,electron-updater 按 `latest.yml.path` 拉取 exe;CI 的 Stage 步骤有硬校验,缺 `latest.yml` 直接红。`differentialPackage:false` 不产 `.blockmap`,故 `updater.ts` 同步设 `autoUpdater.disableDifferentialDownload=true` 走全量下载(否则 updater 仍拉不存在的 `.blockmap` 报 404)。
- **`publish: github` 段在 `electron-builder.win.yml`**：仅用于让 electron-builder 生成元数据，不触发上传（`--publish never`），无需 GH_TOKEN（仓库公开）。
- **每端独立 release，Windows 用 generic feed 适配（load-bearing）**：仓库改为每端独立 GitHub Release（`.github/actions/detect-and-version`），desktop 的 release tag 形如 `desktop-vX.Y.Z`（与 `phone-v*`/`extension-v*`/旧裸 `v*` 并存）。本版 electron-builder/electron-updater（26.15.2 / 6.8.9）**不支持 `tagPrefix`**，且内置 GitHub provider 只会取 GitHub 跨端的「latest」release（可能是 phone/extension）→ 拉错 `latest.yml`。因此 `updater.ts` 在每次检查前自己用 `/releases?per_page=100` 过滤 `^desktop-v?<semver>$`、取版本最大者，Windows 再 `autoUpdater.setFeedURL({provider:"generic", url:"…/releases/download/desktop-vX.Y.Z/"})` 让 generic provider 从该目录拉 `latest.yml` + 安装包；macOS/Linux 用该 release 的 `html_url` 跳转。**不要**改回内置 github provider 的「latest」发现，也**不要**用 `/releases/latest`。
