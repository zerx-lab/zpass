...
/* ============================================================================
 * 「保存登录」toast——右下角丰富 toast，带按钮、可关闭；
 * 锁定状态下的「解锁并保存」toast另走另一个函数。
 * ========================================================================== */

export interface SaveLoginToastInput {
 ...
 * 弹一个「保存登录」 / 「更新密码」 toast。
 *
 * 行为对齐 Bitwarden NotificationBar：
 *   - 右下角常驻，不自动消失（遵 Bitwarden，避免用户未及处理就吃掉凭据）。
 *   - 3 个动作：「保存 / 更新」「永不」「× 关闭」。
 *   - ​uname 不可编辑（避免「保存的账号和页面填的不一样」场景）。
 */
export function showSaveLoginToast(input: SaveLoginToastInput): void {
  const isUpdate = input.decision.status === "update";

  // 同时只保留一个保存 toast——连续提交多次时后一个覆盖前一个。
  document
    .querySelectorAll(".zpass-save-toast")
    .forEach((node) => node.remove());

  const title = isUpdate ? "更新 ZPass 中的密码？" : "保存登录到 ZPass？";
  const subtitle = isUpdate
    ? `${input.decision.itemName || input.decision.origin} 的密码变了。`
    : `保存 ${input.username} 到 ${input.decision.origin}。`;

  const toast = el("div", {
    class: "zpass-save-toast",
    attrs: { role: "dialog", "aria-label": title },
    children: [
      el("div", {
        class: "zpass-save-toast-head",
        children: [
          el("span", {
            class: "zpass-save-toast-icon",
            html: zMatrixIcon({ size: 18 }),
          }),
          el("div", {
            class: "zpass-save-toast-text",
            children: [
      ...
      el("div", {
        class: "zpass-save-toast-actions",
        children: [
          el("button", {
            class: "zpass-save-toast-never",
            attrs: { type: "button", "data-action": "never" },
            text: "永不",
          }),
          el("button", {
            class: "zpass-save-toast-primary",
            attrs: { type: "button", "data-action": "save" },
            text: isUpdate ? "更新密码" : "保存",
          }),
        ],
      }),
    ],
  ...
  toast.addEventListener("click", (event) => {
    ...
    if (action === "save") {
      ...
      if (input.decision.itemId) payload.itemId = input.decision.itemId;
      if (input.suggestedName) payload.name = input.suggestedName;
      browser.runtime
        .sendMessage({ type: "zpass.saveLogin", payload })
        .then((response) => {
          if (response && response.ok === false) {
            // 保存失败——重启按钮使能让用户重试。
            buttons.forEach((b) => (b.disabled = false));
            showPageToast("保存失败", response.error ?? "请重试。");
            return;
          }
          toast.remove();
          showPageToast(
            isUpdate ? "密码已更新" : "已保存到 ZPass",
            input.decision.origin,
          );
        })
        .catch((error: unknown) => {
          buttons.forEach((b) => (b.disabled = false));
...
/**
 * 弹一个「需要解锁 ZPass 才能保存」的 toast。
 *
 * 与「保存」toast 分开：不带 username/password 表单可作为事实上的保存动作，
 * 只提示用户“去解锁” + “稍后”。background 已留住 capture、解锁后会主动
 * push showSaveToast 变 toast。
 */
export function showLockedSaveToast(): void {
  document
    .querySelectorAll(".zpass-save-toast")
    .forEach((node) => node.remove());
  ...
  const toast = el("div", {
    class: "zpass-save-toast",
    attrs: { role: "dialog", "aria-label": "ZPass 锁定中" },
    children: [
      el("div", {
        class: "zpass-save-toast-head",
        children: [
          ...
          el("div", {
            class: "zpass-save-toast-text",
            children: [
              el("strong", { text: "ZPass 已锁定" }),
              el("span", { text: "解锁后可保存这条登录。" }),
            ],
          }),
          el("button", {
            class: "zpass-save-toast-close",
      ...
      el("div", {
        ...
        children: [
          ...
          el("button", {
            class: "zpass-save-toast-primary",
            attrs: { type: "button", "data-action": "unlock" },
            text: "打开 ZPass",
          }),
        ],
      }),
  ...
  toast.addEventListener("click", (event) => {
    ...
    if (button.dataset.action === "unlock") {
      ...
      // 不关闭 toast——让用户仍然看到一个进展提示；background 轮询解锁后
      // 会推 zpass.showSaveToast、到时该函数里 showSaveLoginToast 会 remove
      // 现在这个 toast 再贴上新的。
      return;
    }
    toast.remove();
