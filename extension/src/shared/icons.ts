...
// “复制用户名” —— 人形（lucide user）。区分于密码 mini 按钮的「***」图标，
// 让两个 mini 按钮一眼就能分清。
export function userIcon(size = 14): string {
  return strokeIcon(
    size,
...
// “复制密码” —— 三个星号 ***（与 Bitwarden 现版同语义）。
// 走 fill=currentColor 而不是 stroke，所以独立写，不复用 strokeIcon。
export function asterisksIcon(size = 14): string {
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><circle cx="5" cy="12" r="2.2"/><circle cx="12" cy="12" r="2.2"/><circle cx="19" cy="12" r="2.2"/></svg>`;
}
