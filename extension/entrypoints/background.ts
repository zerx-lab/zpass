...
async function handleMessage(
  ...
  try {
    ...
    if (req.type === "zpass.revealLogin") {
      ...
      const result = await bridge.revealLogin({ origin, url: tab.url, itemId: req.itemId });
      return { ok: true, result };
    }
    ...
    if (req.type === "zpass.fillActiveTab") {
      ...
      const secret = await bridge.revealLogin({ origin, url: tab.url, itemId: req.itemId });
      const fillResponse = await browser.tabs.sendMessage(tab.id, { type: "zpass.fillLogin", secret });
      if (fillResponse && fillResponse.ok === false) {
        return { ok: false, error: fillResponse.error ?? "ZPass 无法填充当前页面。" };
      }
...
async function resolveTab(sender: Browser.runtime.MessageSender): Promise<ActiveTabInfo> {
...
function stringField<T extends object>(payload: Partial<T>, key: keyof T): string {
