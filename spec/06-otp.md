...
| `zpass-vault-service`                | 提供 `advance_hotp_counter(item_id) -> Result<HotpAdvanceResult, _>`：在持有 `hotp_advance_mu` 的串行临界区内完成「读 item → 取 counter → +1 → 写回 item → 同时调 `zpass-otp::hotp` 计算 code」，返回**已生成的 OTP 快照 + 新 counter**给调用方 |
| 调用方（如 UI、native bridge）        | 仅调一次 `vault.advance_hotp_counter(id)` 即拿到完整 OTP 快照，**不**再做第二次 `get_item`（避免与下一次 advance 竞争同一 item） |

返回类型：

```rust
pub struct HotpAdvanceResult {
    pub code: OtpCode,        // 包含 code 字符串、算法、digits 等
    pub new_counter: u64,
    pub item_summary: ItemSummary,  // 顺手让 UI 刷新列表 has_totp 等
}
```
